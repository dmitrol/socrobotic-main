...
const selectedLng = ref<Lang | null>(null)
const lngs = ref<Lang[]>([
    { name: 'русский', value: 'ru' },
    { name: 'английский', value: 'us' },
    { name: 'украинский', value: 'ua' },
    { name: 'испанский', value: 'es' },
    { name: 'ктайский', value: 'chn' },
])
...
onMounted(() => {
    lngs.value.forEach((item) => {
        if (item.value === locale.value) {
            selectedLng.value = item
        }
    })
    watch(selectedLng, async () => {
        if (selectedLng !== null) {
            locale.value = selectedLng.value?.value || 'ru'
            localStorage.setItem('vueml-lang', locale.value)
            await axios.post('config/set', { lang: selectedLng.value?.value })
        }
    })
})
</script>
        ...
        <div class="navbar__right">
            <div class="navbar__right-content">
                <Dropdown
                    ...
                    unstyled
                    :pt="{
                        root: { class: 'dropdown__root' },
                        trigger: { class: 'dropdown__trigger' },
